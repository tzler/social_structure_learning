"""scripts used to analyze data for First Year Project."""
...
class experimental(object):
    ...
    def __init__(self):
        ...
        # configs for low and high pass filtering data
        ...
        #
        self.windowstart = 1
        # set nSeconds to search for amplitude within
        self.nSeconds = 4.5

    def loadData(self):
        """load data from ideosyncratic format."""
    ...
    def transformData(self):
        """Performs high and low pass filter on SCR data."""
        ...
        for iSubject in range(0, len(rawData)):
            ...
            onset = min(min(find(rawData[iSubject][:, 1])),
                    min(find(rawData[iSubject][:, 2]))) - 1000 * self.keepEdgeBefore

            offset = max(max(find(rawData[iSubject][:, 2])),
                     max(find(rawData[iSubject][:, 1]))) + 1000 * self.keepEdgeAfter
            ...
            if std(filteredData) > .1:
                 keepers.append(iSubject)

            # zscore the filtered data
            filteredData = (filteredData-mean(filteredData))/std(filteredData)
    ...
    def simpleDifference(self,inputData,iStim,z = 0,show=0):
        ...
        for iSubject in range(0,len(inputData)):
            ...
            if show:
                ...
                title('CS+ difference is %s, CS- difference is %s, CS+ minus CS- = %s\n%s'%(round(p1[iSubject],3),
                                                                          round(m1[iSubject],3),tmpDifference,self.subjectNames[iSubject]),fontsize=15);
    ...
    def findAmplitude(self,data,stims,iStim,nsForward):
        """simple amplitude difference between t1 and t2."""
